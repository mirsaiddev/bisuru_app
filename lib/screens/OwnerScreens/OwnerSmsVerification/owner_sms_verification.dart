...
class OwnerSmsVerification extends StatefulWidget {
  const OwnerSmsVerification({
    Key? key,
  }) : super(key: key);

  @override
  State<OwnerSmsVerification> createState() => _OwnerSmsVerificationState();
}

class _OwnerSmsVerificationState extends State<OwnerSmsVerification> {
  ...
  Future<void> sendSms() async {
    ...
    UserProvider userProvider = Provider.of<UserProvider>(context, listen: false);
    OwnerModel? ownerModel = userProvider.ownerModel;
    await SmsService().send(number: ownerModel!.phone, text: 'Bi Sürü SMS Doğrulama Kodunuz: $code');
    sent = true;
    setState(() {});
  }
  ...
  void verify() {
    UserProvider userProvider = Provider.of<UserProvider>(context, listen: false);
    OwnerModel? ownerModel = userProvider.ownerModel;
    DatabaseService().verifySms(isUser: false, uid: ownerModel!.uid!);
    ownerModel.smsVerified = true;
